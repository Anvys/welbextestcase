...
export const AddForm: React.FC<TProps> = (props) => {
    ...
    const onFormSubmit: React.FormEventHandler = (e) => {
        e.preventDefault()
        if (!!name && count > 0 && !isNaN(+range) && +range>0) {
            const newRadar = {
                name, count, range:Number(range),
                date: Date.now(),
                id: -1//Заглушка
            }
                ...
                <div className={s.fields}>
                    ...
                    <div>
                        ...
                        <input type={'number'} name={'count'} value={count}
                               onChange={e => setCount(Math.round(+e.target.value))}
                               className={count > 0 ? s.isOK : s.isERR}/>
                    </div>
                    <div>
                        <label htmlFor={'range'}>Range</label>
                        <input type={'text'} name={'range'} value={range}
                               onChange={e => setRange(e.target.value)} className={!isNaN(+range)? s.isOK : s.isERR}/>
                    </div>
                </div>
