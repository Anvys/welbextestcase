        ...
        <div className={s.paginator}>
            <div className={s.main}>
                ...
                {new Array(pages).fill(1).map((v, i) =>(i>=showCount*10 && i<10*showCount+10)?
                    <div key={i} onClick={onPageClick(i+1)} className={page === i + 1 ? s.selected : ''}>{i + 1}</div>:null)}
                {<button onClick={()=>setShowCount(a=>a+1)} disabled={!(showCount*10+10<pages)}>next</button>}
            </div>
